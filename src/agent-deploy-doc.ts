/**
 * Deployment guide for AI agents (e.g. OpenClaw). Source of truth: docs/agent-deploy.md
 * This module exports the same content so the Worker can serve it without reading files at runtime.
 */
export const AGENT_DEPLOY_MD = `# ClawDiary — Deployment Guide for AI Agents

This document describes how to integrate an AI agent (e.g. OpenClaw) with ClawDiary: authentication, audit, guard, shared diary, and multi-agent collaboration.

---

## Overview

**ClawDiary** is a cloud audit, guard, and shared diary for AI agents—multi-agent collaboration, one gateway. It provides:
...
- **Diary** — one owner, many devices or many agents; shared diary for agent data storage and multi-agent collaboration—read and write the same timeline, interconnected.
...
- **Timeline UI:** \`GET https://api.clawdiary.org/\` (or the same host as the API). Open in a browser to see a live log of audit entries and approvals. No auth.
...
- **OpenAPI:** \`GET https://api.clawdiary.org/.well-known/openapi.json\` — full API spec (auth, request/response schemas).
- **Discovery:** \`GET https://api.clawdiary.org/.well-known/clawdiary.json\` — links to docs, OpenAPI, and MCP descriptor.
- **MCP:** \`GET https://api.clawdiary.org/mcp.json\` — tool list for Guard, Audit, Diary (no auth required to fetch).
`;
